...
class PregnancyDashboard extends StatelessWidget {
  ...
  // --- Header ---
  Widget _buildHeader(
      BuildContext context, PregnancyWeekData data, Color color) {
    return Row(
        ...
        Column(
          ...
          children: [
            ...
            Container(
              ...
              child: Text(data.trimester.toUpperCase(),
  ...
  Widget _buildMilestoneHero(
      BuildContext context, _PregnancyInfo info, double progress, Color color) {
    return Container(
      height: 240,
      decoration: BoxDecoration(
            ...
            Row(
              ...
              children: [
                ...
                Column(
                  ...
                  children: [
                    Text('${info.daysLeft} days left',
                        style: GoogleFonts.inter(
                            fontSize: 14,
                            fontWeight: FontWeight.w800,
                            color: Colors.white)),
                    const SizedBox(height: 4),
                    Text(DateFormat('MMM d, yyyy').format(info.dueDate),
                        style: GoogleFonts.inter(
  ...
  Widget _buildWeeklySpotlight(PregnancyWeekData data, Color color) {
    return Row(
      children: [
        Expanded(
            child: _spotlightCard('Baby', data.milestone, color,
                'assets/images/baby_placeholder.png')),
        const SizedBox(width: 12),
        Expanded(
            child: _spotlightCard(
                'Body', data.bodyUpdate, AppTheme.accentPurple, null)),
      ],
    );
  }

  Widget _spotlightCard(
      String title, String text, Color color, String? imgPath) {
    return Container(
  ...
  Widget _buildHealthTracker(StorageService storage, Color color) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          _metricTile('Water', '${storage.getHydrationToday()}/15',
              Icons.water_drop, Colors.blueAccent),
          _metricTile('Steps', '${storage.getStepsToday()}',
              Icons.directions_walk, Colors.orangeAccent),
          _metricTile('Sleep', '${storage.getSleepHours()}h', Icons.bedtime,
              AppTheme.accentPurple),
          _metricTile('Mood', storage.getMoodToday(), Icons.favorite,
              AppTheme.accentPink),
        ],
      ),
    );
      ...
      child: Column(
        children: [
          ...
          ElevatedButton(
            ...
            child: Text('Setup Start Date (LMP)',
          ...
          children: [
            Text('Edit Journey',
                style: GoogleFonts.poppins(
                    ...
                    color: AppTheme.textDark)),
            const SizedBox(height: 12),
            Text('Recalibrate your weeks based on your Last Period (LMP).',
                style: GoogleFonts.inter(
            ...
            ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(
                  ...
                  shape: RoundedRectangleBorder(
                      borderRadius: BorderRadius.circular(20))),
              child: Text('Update Last Period (LMP)',
                  style: GoogleFonts.inter(fontWeight: FontWeight.w800)),
            ),
            const SizedBox(height: 16),
            TextButton(
