...
void main() {
  group('PredictionService Tests', () {
    ...
    test('averageCycleLength returns 28 for less than 2 logs', () {
      ...
      fakeStorage.logs = [
        PeriodLog(startDate: DateTime.now(), duration: 5)
      ];
      expect(predictionService.averageCycleLength, 28);
    });

    test('averageCycleLength correctly calculates average of cycles and ignores outliers', () {
      final now = DateTime.now();
      // Add logs: newest to oldest
      fakeStorage.logs = [
        PeriodLog(startDate: now, duration: 5), // Cycle 1 start
        PeriodLog(startDate: now.subtract(const Duration(days: 30)), duration: 5), // Cycle 2 start (30 day diff)
        PeriodLog(startDate: now.subtract(const Duration(days: 58)), duration: 5), // Cycle 3 start (28 day diff)
        PeriodLog(startDate: now.subtract(const Duration(days: 158)), duration: 5), // outlier (100 day diff! ignored)
      ];
    ...
    test('currentPhase identifies menstrual phase accurately', () {
      final now = DateTime.now();
      fakeStorage.logs = [
        PeriodLog(startDate: now.subtract(const Duration(days: 2)), duration: 5)
      ];
    ...
    test('currentPhase identifies follicular phase accurately', () {
      ...
      fakeStorage.logs = [
        ...
        PeriodLog(startDate: now.subtract(const Duration(days: 8)), duration: 5)
      ];
    ...
    test('currentPhase identifies luteal phase accurately', () {
      ...
      fakeStorage.logs = [
        ...
        PeriodLog(startDate: now.subtract(const Duration(days: 20)), duration: 5)
      ];
    ...
    test('isFertileDay correctly flags days within the 5-day ovulation window', () {
      final now = DateTime.now();
      fakeStorage.logs = [
        PeriodLog(startDate: now, duration: 5),
        PeriodLog(startDate: now.subtract(const Duration(days: 28)), duration: 5), // average 28
      ];

      final day5 = now.add(const Duration(days: 5));  // Safe
      final day10 = now.add(const Duration(days: 10)); // Fertile (14 - 4)
      final day14 = now.add(const Duration(days: 14)); // Peak Ovulation (Fertile)
